...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, use_cls=True, **kwargs):
        super().__init__()
    ...
    def forward(self, x, patch_inds=None):
        ...
        if not isinstance(x, list):
            x = [x]
            patch_inds = [patch_inds] if patch_inds is not None else None
        ...
        for end_idx in idx_crops:
            if patch_inds is None:
                _out = self.forward_features(torch.cat(x[start_idx: end_idx]))
            else:
                _out = self.forward_features(torch.cat(x[start_idx: end_idx]),
                                             torch.cat(patch_inds[start_idx: end_idx]))
            if start_idx == 0:
                output = _out
            else:
    ...
    def forward_return_n_last_blocks(self, x, n=1, return_patch_avgpool=False, patch_inds=None):
        B = x.shape[0]
        x = self.patch_embed(x)
